...
static void sigmap_cleanup(void) {
  ...
  SFREE(ovec);
  if (smi) {
    for (i = 0; i < tspf; i += smi[i].spf) SFREE(smi[i].desc);
    SFREE(smi);
  }
...
static int sigmap_init(int first_segment) {
  ...
  if (in_msrec && first_segment && segarray && segarray[0].nsamp == 0) {
    ...
    nvsig = nisig;
    vspfmax = ispfmax;
    for (s = tspf = 0; s < nisig; s++) tspf += isd[s]->info.spf;
        ...
        if (strcmp(smi[s].desc, isd[i]->info.desc) == 0) {
          if ((kmax = smi[s].spf) != isd[i]->info.spf) {
            wfdb_error(
                ...
                segp->recname);
            if (kmax > isd[i]->info.spf) kmax = isd[i]->info.spf;
          }
          for (k = 0; k < kmax; k++) {
            ps = &smi[s + k];
            ps->index = j + k;
            ...
            if (ps->scale < 1.0)
              wfdb_error(
                  ...
                  i, segp->recname);
            ps->offset = ps->baseline - ps->scale * isd[i]->info.baseline + 0.5;
            ...
            if (ovmin < ovmax && ovmin >= WFDB_SAMPLE_MIN + 1 &&
    ...
    for (i = 0; i < nisig; i++) {
      if (isd[i]->info.spf != vsd[i]->info.spf) {
        wfdb_error("sigmap_init: wrong spf for signal %d in segment %s\n", i,
                   segp->recname);
        return (-1);
      }
    }
...
static int edfparse(WFDB_FILE *ifile) {
  ...
  for (s = 0; s < nsig; s++) {
    ...
    for (i = 0; adcrange > 0; i++) adcrange /= 2;
    hsd[s]->info.adcres = i;
    if (pmax[s] != pmin[s]) {
  ...
  gvmode |= WFDB_HIGHRES;
  sprintf(buf, "%02d:%02d:%02d %02d/%02d/%04d", hour, minute, second, day,
          month, year);
  setbasetime(buf);
...
static int readheader(const char *record) {
  ...
  if (strncmp("#wfdb", linebuf, 5) == 0) { /* found the magic string */
    ...
    if ((i > 0 && major > WFDB_MAJOR) ||
      ...
      wfdb_error(
          "init: reading record %s requires WFDB library "
  ...
  if (p = strtok((char *)NULL, sep)) {
    if ((f = (WFDB_Frequency)strtod(p, NULL)) <= (WFDB_Frequency)0.) {
      wfdb_error("init: sampling frequency in record %s header is incorrect\n",
                 record);
      return (-2);
    }
  ...
  if (segments && !in_msrec) {
    ...
    for (i = 0, ns = (WFDB_Time)0L; i < segments; i++, segp++) {
      ...
      do {
        if (wfdb_fgets(linebuf, 256, hheader) == NULL) {
          wfdb_error("init: unexpected EOF in header file for record %s\n",
                     record);
          SFREE(segarray);
          segments = 0;
          return (-2);
      ...
      if (*p == '+') {
        wfdb_error("init: `%s' is not a valid segment name in record %s\n", p,
                   record);
        SFREE(segarray);
        segments = 0;
        return (-2);
      ...
      if (strlen(p) > WFDB_MAXRNL) {
        wfdb_error("init: `%s' is too long for a segment name in record %s\n",
                   p, record);
        SFREE(segarray);
        segments = 0;
        return (-2);
    ...
    if (msnsamples == 0L)
      ...
      wfdb_error(
          "warning (init): in record %s, "
          "stated record length (%" WFDB_Pd_TIME ")\n",
          record, msnsamples);
      wfdb_error(
          " does not match sum of segment lengths "
          "(%" WFDB_Pd_TIME ")\n",
          ns);
    }
    return (0);
  ...
  for (s = 0; s < nsig; s++) {
    ...
    hs->info.adczero =
        (p = strtok((char *)NULL, sep)) ? strtol(p, NULL, 10) : 0;
    ...
    /* Determine the initial value (assumed to be equal to the ADC
       zero if missing). */
    hs->info.initval = (p = strtok((char *)NULL, sep)) ? strtol(p, NULL, 10)
                                                       : hs->info.adczero;
    ...
    if (p = strtok((char *)NULL, "\n\r"))
      ...
      (void)sprintf(hs->info.desc, "record %s, signal %d", record, s);
  }
...
static int osigclose(void) {
  ...
  for (g = 0; g < nogroup; g++)
    if (ogd && (og = ogd[g])) og->force_flush = 1;
  ...
  if (oheader) {
    ...
    if (wfdb_fclose(oheader)) errflag = 1;
    if (errflag) {
...
static int isgsetframe(WFDB_Group g, WFDB_Time t) {
  ...
  if (nigroup < 2 && istime == t && gvc == ispfmax && igd[g]->start == 0)
    return (0);
  ...
  if (in_msrec) {
    ...
    while (t < tseg->samp0) tseg--;
    while (t >= tseg->samp0 + tseg->nsamp && tseg < segend) tseg++;
    if (segp != tseg) {
      segp = tseg;
  ...
  switch (isd[s]->info.fmt) {
    ...
    case 212:
      ...
      if ((nn & 1) && (t & 1)) {
        if (in_msrec) t += segp->samp0; /* restore absolute time */
        if (i = isgsetframe(g, t - 1)) return (i);
        for (j = 0; j < nn; j++) (void)r212(ig);
        istime++;
    ...
    case 310:
      ...
      if ((nn % 3) && (trem = (t % 3))) {
        if (in_msrec) t += segp->samp0; /* restore absolute time */
        if (i = isgsetframe(g, t - trem)) return (i);
        for (j = nn * trem; j > 0; j--) (void)r310(ig);
        istime += trem;
        for (n = 0; s + n < nisig && isd[s + n]->info.group == g; n++)
          isd[s + n]->info.nsamp = (WFDB_Time)0L;
        return (0);
    ...
    case 311:
      ...
      if ((nn % 3) && (trem = (t % 3))) {
        if (in_msrec) t += segp->samp0; /* restore absolute time */
        if (i = isgsetframe(g, t - trem)) return (i);
        for (j = nn * trem; j > 0; j--) (void)r311(ig);
        istime += trem;
        for (n = 0; s + n < nisig && isd[s + n]->info.group == g; n++)
          isd[s + n]->info.nsamp = (WFDB_Time)0L;
        return (0);
  ...
  if (s == 0) istime = in_msrec ? t + segp->samp0 : t;
  while (n-- != 0) isd[s + n]->info.nsamp = (WFDB_Time)0L;
  return (0);
}
...
static int getskewedframe(WFDB_Sample *vector) {
  ...
  for (s = 0; s < nisig; s++) {
    ...
    if (is->info.nsamp >= 0 && --is->info.nsamp == 0 &&
        (is->info.cksum & 0xffff) && !in_msrec && !isedf && is->info.fmt != 0) {
  ...
  if (need_sigmap)
    ...
    for (s = framelen; s < tspf; s++) vecstart[s] = WFDB_INVALID_SAMPLE;
...
static WFDB_Sample meansamp(const WFDB_Sample *s, int n) {
  ...
  if (WFDB_SAMPLE_MAX <= WFDB_TIME_MAX / INT_MAX) {
    ...
    for (i = 0; i < n; i++) {
      if (*s == WFDB_INVALID_SAMPLE) return (WFDB_INVALID_SAMPLE);
      sum += *s++;
    }
    if (n & (n - 1)) { /* if n is not a power of two */
      return (sum + (sum < 0)) / n - (sum < 0);
    } else {
      while ((n /= 2) != 0) sum = (sum - (sum < 0)) / 2;
      return sum;
    }
  }
...
int isigopen(char *record, WFDB_Siginfo *siarray, int nsig) {
  ...
  if (nsig <= 0) {
    ...
    if (siarray != NULL)
      for (s = 0; s < nsig; s++) siarray[s] = hsd[s]->info;
  ...
  if (nn > nigroup + nsig) nn = nigroup + nsig;
  if (allocigroup(nn) != nn) return (-1); /* failed, allocigroup emits error */
  ...
  for (si = 0; si < s; si++) {
    is = isd[nisig + si];
    if (siarray) copysi(&siarray[si], &is->info);
  ...
  for (si = framelen = 0; si < nisig; si++) framelen += isd[si]->info.spf;
...
static int openosig(const char *func, WFDB_Siginfo *si_out,
  ...
  for (s = 0, os = osd[nosig]; s < nsig; s++, nosig++, si_in++) {
    ...
    if (s == 0 || os->info.group != op->info.group) {
      ...
      if (os->info.fmt != op->info.fmt || os->info.bsize != op->info.bsize) {
        wfdb_error("%s: error in specification of signal %d or %d\n", func,
                   s - 1, s);
        return (-2);
      }
    }
...
int osigopen(char *record, WFDB_Siginfo *siarray, unsigned int nsig) {
  ...
  if ((n = readheader(record)) < 0) return (n);
  if (n < nsig) {
  ...
  if (!hsi) return (-3);
  for (s = 0; s < nsig; s++) hsi[s] = hsd[s]->info;
...
int osigfopen(const WFDB_Siginfo *siarray, unsigned int nsig) {
  ...
  for (s = 0, si = siarray; s < nsig; s++, si++) {
    ...
    if (strlen(si->fname) + strlen(si->desc) > 200 || si->bsize < 0 ||
        !isfmt(si->fmt)) {
    ...
    if (!((s == 0 && si->group == 0) ||
          ...
          (s && si->group == (si - 1)->group + 1 &&
           strcmp(si->fname, (si - 1)->fname) != 0))) {
      wfdb_error("osigfopen: incorrect file name or group for signal %d\n", s);
      return (-2);
    }
  }
...
int getframe(WFDB_Sample *vector) {
  ...
  if (dsbuf) { /* signals must be deskewed */
    ...
    for (j = s = 0; s < nvsig; s++) {
      ...
      for (c = 0; c < vsd[s]->info.spf; c++) vector[j++] = dsbuf[i++];
    }
...
int putvec(const WFDB_Sample *vector) {
  ...
  for (s = 0; s < nosig; s++) {
    ...
    for (c = 0; c < os->info.spf; c++, vector++) {
      ...
      if ((samp = *vector) == WFDB_INVALID_SAMPLE) switch (os->info.fmt) {
          case 0:
          case 8:
          case 16:
          case 61:
  ...
  if (in_msrec && need_sigmap) { /* variable-layout multi-segment record */
    ...
    while (stat >= 0) {
      ...
      for (ss = 0; ss < nisig; ss++)
        if ((q = isd[ss]->info.desc) && strcmp(p, q) == 0) break;
      if (ss < nisig) {
...
int newheader(char *record) {
  ...
  for (s = 0; s < nosig; s++) copysi(&osi[s], &osd[s]->info);
...
int setheader(char *record, const WFDB_Siginfo *siarray, unsigned int nsig) {
  ...
  if (wfdb_checkname(record, "record")) return (-1);
  ...
  if ((cfreq > 0.0 && cfreq != ffreq) || bcount != 0.0) {
    ...
    if (bcount != 0.0) (void)wfdb_fprintf(oheader, "(%.12g)", bcount);
  }
  ...
  if (btime != 0L || bdate != (WFDB_Date)0) {
    if (btime == 0L)
      ...
      (void)wfdb_fprintf(oheader, " %s", ftimstr(btime, 1000.0));
    else
      (void)wfdb_fprintf(oheader, " %s", fmstimstr(btime, 1000.0));
  }
  if (bdate) (void)wfdb_fprintf(oheader, "%s", datstr(bdate));
  ...
  for (s = 0; s < nsig; s++) {
    ...
    if (siarray[s].spf > 1) (void)wfdb_fprintf(oheader, "x%d", siarray[s].spf);
    if (osd && osd[s]->skew)
      (void)wfdb_fprintf(oheader, ":%d", osd[s]->skew * siarray[s].spf);
    if (ogd && ogd[osd[s]->info.group]->start)
      (void)wfdb_fprintf(oheader, "+%ld", ogd[osd[s]->info.group]->start);
    ...
    (void)wfdb_fprintf(oheader, " %d %d %d %d %d", siarray[s].adcres,
                       siarray[s].adczero, siarray[s].initval,
                       (short int)(siarray[s].cksum & 0xffff),
                       siarray[s].bsize);
...
int setmsheader(char *record, char **segment_name, unsigned int nsegments) {
  ...
  for (i = 0; i < nsegments; i++) {
    ...
    if (i == 0) {
      ...
      if (nsig != n) {
        wfdb_error("setmsheader: incorrect number of signals in segment %s\n",
                   segment_name[i]);
        SFREE(ns);
        return (-4);
      }
      if (msfreq != ffreq) {
        wfdb_error("setmsheader: incorrect sampling frequency in segment %s\n",
                   segment_name[i]);
        SFREE(ns);
        return (-4);
      }
  ...
  (void)wfdb_fprintf(oheader, "%s/%u %d %.12g", record, nsegments, nsig,
                     msfreq);
  if ((mscfreq > 0.0 && mscfreq != msfreq) || msbcount != 0.0) {
    (void)wfdb_fprintf(oheader, "/%.12g", mscfreq);
    if (msbcount != 0.0) (void)wfdb_fprintf(oheader, "(%.12g)", msbcount);
  }
  (void)wfdb_fprintf(oheader, " %" WFDB_Pd_TIME, msnsamples);
  if (msbtime != 0L || msbdate != (WFDB_Date)0) {
    if (msbtime % 1000 == 0)
      (void)wfdb_fprintf(oheader, " %s", ftimstr(msbtime, 1000.0));
    else
      (void)wfdb_fprintf(oheader, " %s", fmstimstr(msbtime, 1000.0));
  }
  if (msbdate) (void)wfdb_fprintf(oheader, "%s", datstr(msbdate));
  (void)wfdb_fprintf(oheader, "\r\n");
...
int setbasetime(char *string) {
  ...
  if (string == NULL || *string == '\0') {
    ...
    now = localtime(&t);
    (void)sprintf(date_string, "%02d/%02d/%d", now->tm_mday, now->tm_mon + 1,
                  now->tm_year + 1900);
    bdate = strdat(date_string);
    (void)sprintf(time_string, "%d:%d:%d", now->tm_hour, now->tm_min,
                  now->tm_sec);
  ...
  while (*string == ' ') string++;
  if (p = strchr(string, ' ')) *p++ = '\0'; /* split time and date components */
...
static char *fmstimstr(WFDB_Time t, WFDB_Frequency f) {
  ...
  if (t > 0L || (btime == 0L && bdate == (WFDB_Date)0)) { /* time interval */
    ...
    hours = t / 60;
    if (hours > 0)
      (void)sprintf(time_string, "%2d:%02d:%02d.%03d", hours, minutes, seconds,
                    msec);
    else
      (void)sprintf(time_string, "   %2d:%02d.%03d", minutes, seconds, msec);
  } else { /* time of day */
    ...
    (void)sprintf(time_string, "[%02d:%02d:%02d.%03d%s]", hours, minutes,
                  seconds, msec, date_string);
  }
  return (time_string);
}
...
static WFDB_Time fstrtim(const char *string, WFDB_Frequency f) {
  ...
  switch (*string) {
    ...
    case 'f':
      return (WFDB_Time)(strtotime(string + 1, NULL, 10) * f / ffreq);
    case 'i':
      return (
          WFDB_Time)(istime * (ifreq > 0.0 ? (ifreq / sfreq) : 1.0) *
                     (((gvmode & WFDB_HIGHRES) == WFDB_HIGHRES) ? ispfmax : 1));
    case 'o':
      return (ostime);
    case 's':
      return (strtotime(string + 1, NULL, 10));
...
void wfdb_freeinfo() {
  ...
  for (i = 0; i < nimax; i++) SFREE(pinfo[i]);
  SFREE(pinfo);
  nimax = ninfo = 0;
}
...
int wfdb_oinfoclose() {
  ...
  if (outinfo && outinfo != oheader) {
    ...
    if (wfdb_fclose(outinfo)) errflag = 1;
    if (errflag) {
