...
static WFDB_Time round_to_time(double x) {
  ...
  if (x >= 0) {
    if (x >= WFDB_TIME_MAX) return (WFDB_TIME_MAX);
    t = x;
    if (x - t >= 0.5)
      return (t + 1);
    else
    ...
    if (x <= WFDB_TIME_MIN) return (WFDB_TIME_MIN);
    t = x;
    if (x - t < -0.5)
      return (t - 1);
    else
...
static int get_ann_table(WFDB_Annotator i) {
  ...
  while (getann(i, &annot) == 0 && annot.time == 0L && annot.anntyp == NOTE &&
         annot.subtyp == 0) {
    if (annot.aux == NULL || *annot.aux < 1) continue;
    if (*(annot.aux + 1) == '#') {
...
int annopen(char *record, const WFDB_Anninfo *aiarray, unsigned int nann) {
  ...
  if (nann == 0) return (annclose_error);
  ...
  niafneeded = niaf;
  noafneeded = noaf;
  for (i = 0; i < nann; i++) switch (aiarray[i].stat) {
      ...
      case WFDB_AHA_WRITE: /* AHA-format output file */
        ...
        break;
      default:
        wfdb_error("annopen: illegal stat %d for annotator %s, record %s\n",
                   aiarray[i].stat, aiarray[i].name, record);
        return (-5);
    }
  /* Allocate workspace. */
  if (allociann(niafneeded) < 0 || allocoann(noafneeded) < 0) return (-3);
  ...
  for (i = 0; i < nann; i++) { /* open the annotation files */
    ...
    switch (aiarray[i].stat) {
      ...
      case WFDB_AHA_READ: /* AHA-format input file */
        ...
        a = (ia->word >> 8) & 0xff;
        if ((ia->word & 0xff) || ammap(a) == NOTQRS || a == '[' || a == ']') {
      ...
      case WFDB_AHA_WRITE: /* AHA-format output file */
        ...
        if (wfdb_checkname(aiarray[i].name, "annotator")) return (-4);
...
int getann(WFDB_Annotator n, WFDB_Annotation *annot) {
  ...
  if (ia->ateof) {
    if (ia->ateof != -1) return (-1); /* reached logical EOF */
  ...
  *annot = ia->ann;
      ...
      while (((ia->word = (unsigned)wfdb_g16(ia->file)) & CODE) >= PAMIN &&
        ...
        switch (ia->word & CODE) { /* process pseudo-annotations */
          case SKIP:
            ...
            break;
          case SUB:
            ia->ann.subtyp = DATA & ia->word;
            break;
          case CHN:
            ia->ann.chan = DATA & ia->word;
            break;
          case NUM:
            ia->ann.num = DATA & ia->word;
            break;
          case AUX:                /* auxiliary information */
            ...
            (void)wfdb_fread(ia->auxstr + ia->index, 1, (len + 1) & ~1,
                             ia->file);
  ...
  if (wfdb_feof(ia->file)) ia->ateof = -1;
  return (0);
}
...
int ungetann(WFDB_Annotator n, const WFDB_Annotation *annot) {
  ...
  if (iad[n]->pann.anntyp) {
    ...
    wfdb_error("ungetann: annotation at %" WFDB_Pd_TIME
               ", annotator %d "
               "not pushed back\n",
               annot->time, n);
    return (-1);
  ...
  switch (oa->info.stat) {
    ...
    default:
      ...
      if (t > oa->ann.time) {
        ...
        while (delta > MAXSKIP) {
          wfdb_p16(SKIP, oa->file);
          wfdb_p32(MAXSKIP, oa->file);
          delta -= MAXSKIP;
        }
      } else {
      ...
      if (annot->aux != NULL && *annot->aux != 0) {
        ...
        if (*annot->aux & 1) (void)wfdb_putc('\0', oa->file);
      }
      break;
    case WFDB_AHA_WRITE: /* AHA-format output file */
      ...
      if (ap = annot->aux)
        ...
        len = 0;
      for (i = 0, ap++; i < len; i++, ap++) (void)wfdb_putc(*ap, oa->file);
      for (; i < AUXLEN; i++) (void)wfdb_putc('\0', oa->file);
      break;
  }
...
static char *cstring[ACMAX + 1] = {
    /* ECG mnemonics for each code */
    " ",    "N",    "L",    "R",    "a",    /* 0 - 4 */
    "V",    "F",    "J",    "A",    "S",    /* 5 - 9 */
    "E",    "j",    "/",    "Q",    "~",    /* 10 - 14 */
    "[15]", "|",    "[17]", "s",    "T",    /* 15 - 19 */
...
int strecg(const char *str) {
  ...
  for (code = 1; code <= ACMAX; code++)
    if (strcmp(str, cstring[code]) == 0) return (code);
  return (NOTQRS);
}
...
int strann(const char *str) {
  ...
  if (str == NULL) str = "";
  for (code = 1; code <= ACMAX; code++)
    if (strcmp(str, astring[code]) == 0) return (code);
  return (NOTQRS);
}
  ...
  if (n < noaf && (oa = oad[n]) != NULL && oa->file != NULL) {
    switch (oa->info.stat) {
      ...
      case WFDB_AHA_WRITE: /* write logical EOF for AHA-format files */
        ...
        while (i-- > 0) (void)wfdb_putc(EOAF, oa->file);
        break;
    }
    errflag = wfdb_ferror(oa->file);
    if (wfdb_fclose(oa->file)) errflag = 1;
    if (errflag) {
    ...
    if (oa->out_of_order) {
      ...
      if (dosort) {
        if (system(NULL) != 0) {
          wfdb_error("Rearranging annotations for output annotator %s ...",
                     oa->info.name);
          ...
          wfdb_asprintf(&cmdbuf, "sortann -r. %s -a %s", oa->rname,
                        oa->info.name);
          ...
          SFREE(cmdbuf);
          if (annclose_error == 0) annclose_error = -6;
        }
      }
    }
    if (oa->out_of_order) {
...
void wfdb_oaflush() {
  ...
  for (i = 0; i < noaf; i++) (void)wfdb_fflush(oad[i]->file);
}
